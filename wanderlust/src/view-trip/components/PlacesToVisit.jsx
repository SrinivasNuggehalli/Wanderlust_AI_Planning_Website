...
function PlacesToVisit({ trip, onSelectPlace, selectedPlaces }) {
  ...
  useEffect(() => {
    if (trip?.tripData?.itinerary) {
      ...
      if (Array.isArray(itineraryData)) {
        ...
        setItinerary(Object.values(itineraryData));
      } else {
  ...
  const replacePlace = async (dayIndex, placeIndex, e) => {
    ...
    try {
      ...
      const allPlaceNames = itinerary.flatMap(day => day.plan.map(place => place.placeName));
      const oldPlaceName = itinerary[dayIndex].plan[placeIndex].placeName;

      // Exclude the old place name (since we're replacing it)
      const existingPlaceNames = allPlaceNames.filter(name => name !== oldPlaceName);
      ...
      while (attempts < maxAttempts) {
        ...
        if (potentialPlace && !existingPlaceNames.includes(potentialPlace.placeName)) {
      ...
      if (newPlace) {
        ...
        if (dayPlan && Array.isArray(dayPlan.plan)) {
          ...
          if (selectedPlaces.some(p => p.placeName === oldPlace.placeName)) {
        ...
        console.error('Could not find a new place that is not already in the itinerary.');
      }
    } catch (error) {
